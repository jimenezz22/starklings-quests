...
mod ContractA {
    ...
    impl ContractAImpl of super::IContractA<ContractState> {
        fn set_value(ref self: ContractState, value: u128) -> bool {
            ...
            let enable = IContractBDispatcher { contract_address: self.contract_b.read() }.is_enabled(); //checking if a certain contract (Contract B) is enabled, and storing the result in a variable.
