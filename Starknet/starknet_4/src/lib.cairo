...
//This is a bit challenging for Joe and Jill, Liz prepared an outline for how contract should work, 
//can you help Jill and Joe write it?
...
mod LizInventory {
    ...
    impl LizInventoryImpl of super::ILizInventory<ContractState> {
        fn add_stock(ref self: ContractState, product: felt252, new_stock: u32) {
            ...
            if (caller == self.contract_owner.read()) {

                let current_stock = self.inventory.read(product); // get current stock
                let stock = current_stock + new_stock; //update stock
                self.inventory.write(product, stock); // write stock back to storage

            } else {
                panic!("Only owner can add stock to inventory");
            }
        }
        ...
        fn purchase(ref self: ContractState, product: felt252, quantity: u32) {
            ...
            //Such as anybody can call this, we don't need to check caller

            let current_stock = self.inventory.read(product); // get current stock
            let new_stock = current_stock - quantity; //update stock
            self.inventory.write(product, new_stock); // write stock back to storage
        }

        fn get_stock(self: @ContractState, product: u32) -> u32 {
            // TODO:
            // * takes product
