...
mod ProgressTracker {
    ...
    struct Storage {
        ...
        // TODO: Set types for LegacyMap. LegacyMap is a map that stores the progress of each user.
    ...
    impl ProgressTrackerImpl of super::IProgressTracker<ContractState> {
        fn set_progress(ref self: ContractState, user: ContractAddress, new_progress: u16) { 
            ...
            let caller = get_caller_address(); // Get caller address is a function that returns the address of the caller

            if (caller == self.contract_owner.read()) { // Check if caller is the owner

                self.progress.write(user, new_progress); // Set progress for user

            } else {
                panic!("Only contract's owner can modify storage");
            }
        }
